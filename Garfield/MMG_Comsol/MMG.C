...
int main(int argc, char * argv[]) {
  ...
  std::vector<std::string> MMG_list = {"MMG1","MMG2","MMG3","MMG4"};
  std::vector<std::string> edrift_list;
  for(int fields = 50;fields<601;fields+=50) edrift_list.emplace_back(std::to_string(fields));
  //std::vector<std::string> edrift_list = {"300"}; 
  //std::string edrift = "0";
  std::string mmgv = "440";  

  for(auto MMG : MMG_list){
    ...
    for(auto edrift : edrift_list){
      ...
      fm.Initialise(MMG+"_mesh.mphtxt", "dielectrics.dat", MMG+"_outfile_"+std::to_string(count)+".txt", "mm");
      ...
      if (plotField) {
        ...
        cf->SaveAs(("../Results/"+MMG+"/plot_field_"+MMG+"_Edrift"+edrift+"_MMG"+mmgv+".png").c_str());
      }
      ...
      if (plotDrift) {
        ...
        cd->SaveAs(("../Results/"+MMG+"/plot_drift_"+MMG+"_Edrift"+edrift+"_MMG"+mmgv+".png").c_str());
      }
